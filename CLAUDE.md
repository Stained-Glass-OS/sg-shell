...
| `sg-control` | **Control Panel.** A window showing the machine's real state -- edition, OS build, computer name, system type, processor and installed RAM, the signed-in user and whether they are an administrator in this session, how Windows Update is managed, how many machine policies are in force, and the count of installed programs. Reads the live token, registry and system, not canned strings (`--dump` prints them for the gate). `control.exe` resolves to it via App Paths (`defaults/61-sg-control-panel.reg`); sg-start lists it. |
