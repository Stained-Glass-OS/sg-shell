 ...
 * (see sg-session); the console only asks and reports the answer. Creating,
 * deleting and resizing partitions are not offered.
 *
...
static BOOL g_admin;
enum { V_RESCAN = 1, V_OPEN, V_LETTER, V_FORMAT, V_MOUNT, V_UNMOUNT, V_PROPS };
...
static void paint_graph(HWND h)
{
    ...
    for (i = 0; i < g_ndisks; i++)
    {
        ...
        }
        {
            ...
            t.left += S(6);
            SelectObject(dc, g_font);
            DrawTextW(dc, L"Online", -1, &t, DT_SINGLELINE | DT_NOPREFIX);
        }
        for (s = 0; s < g_nsegs; s++)
        {
            seg_t *g = &g_segs[s];
...
static void verbs_menu(int s)
{
    ...
    POINT pt;
    int i, cmd;
    disk_verbs(node_current(), s + 1, TRUE, &vs);
    for (i = 0; i < vs.n; i++)
    {
...
static void disk_open(node_t *n, LPARAM key)
{
    int s = seg_of_key(key);
    (void)n;
    if (s >= 0 && !g_segs[s].kind) properties(s);
}
